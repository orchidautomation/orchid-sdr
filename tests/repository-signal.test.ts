...
import { OrchidRepository } from "../src/repository.js";

describe("OrchidRepository.insertSignal", () => {
  it("returns the existing signal id when an upsert hits the source/source_ref conflict", async () => {
    const query = vi.fn().mockResolvedValue({
      rows: [{ id: "sig_existing" }],
    });
    const repository = new OrchidRepository({ query } as unknown as Pool);
  ...
  it("sanitizes noisy audience metrics before writing signal titles", async () => {
    const query = vi.fn().mockResolvedValue({
      rows: [{ id: "sig_existing" }],
    });
    const repository = new OrchidRepository({ query } as unknown as Pool);

    await repository.insertSignal({
      id: "sig_new",
      campaignId: "cmp_default",
      source: "linkedin_public_post",
      sourceRef: "post-123",
