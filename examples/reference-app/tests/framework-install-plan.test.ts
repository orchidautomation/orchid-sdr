...
describe("framework module install plans", () => {
  it("summarizes how a module would be added", () => {
    ...
    expect(plan.packageName).toBe("@trellis/attio");
  ...
  it("resolves capability and provider pairs", () => {
    ...
    expect(findModuleForAddCommand(modules, { capabilityOrModule: "mcp", provider: "trellis-mcp" })?.id).toBe("trellis-mcp");
  });

  it("indexes MCP servers and tool capabilities", () => {
    const parallelPlan = buildModuleInstallPlan(
      findModuleForAddCommand(defaultTrellisModules(), { capabilityOrModule: "deep-research", provider: "parallel" })!,
    );
    const firecrawlPlan = buildModuleInstallPlan(
      findModuleForAddCommand(defaultTrellisModules(), { capabilityOrModule: "extract", provider: "firecrawl" })!,
    );
