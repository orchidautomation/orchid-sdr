...
async function buildResearchBrief(
  ...
  const turn = await deps.runSandboxTurn(
    buildTurnRequest(snapshot, "build_research_brief", [
      ...
      "Use the trellis MCP tools, Parallel MCP tools, Firecrawl MCP tools, and local knowledge files when useful.",
...
async function draftFirstOutbound(
  ...
  const turn = await deps.runSandboxTurn(
    buildTurnRequest(snapshot, "first_outbound", [
      ...
      "Use the trellis MCP tools and local knowledge files when useful.",
...
function buildTurnRequest(
  ...
  return {
    ...
    systemPrompt: [
      ...
      "You can use trellis MCP tools for CRM context, research, enrichment, email, and handoff.",
