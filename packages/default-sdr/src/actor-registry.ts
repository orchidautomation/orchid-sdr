...
export interface DefaultSdrSandboxTurnResponse {
  turnId: string;
  outputText: string;
  transcript: unknown[];
...
export interface DefaultSdrRepositoryContext {
  repository: {
    ...
    getControlFlags(): Promise<{ pausedCampaignIds: string[] }>;
    setCampaignLinkedinSource(campaignId: string, enabled: boolean): Promise<void>;
  };
}

export interface DefaultSdrRegistryDependencies<Context extends DefaultSdrRepositoryContext> {
  getContext(): Context;
  ...
  ): Promise<DefaultSdrInboundReplyOutcome | null>;
  runSandboxTurn(context: Context, request: DefaultSdrSandboxTurnRequest): Promise<DefaultSdrSandboxTurnResponse>;
  discoveryCoordinator: any;
  managerHost?: string;
  managerPort?: number;
}
...
export function createDefaultSdrRegistry<Context extends DefaultSdrRepositoryContext>(
    ...
    actions: {
      bootstrapFromSignal: async (
        ...
        const campaign = input.campaignId
          ? await context.repository.ensureDefaultCampaign().then((defaultCampaign) => ({
              id: input.campaignId ?? defaultCampaign.id,
            }))
        ...
        const outcome = await deps.executeProspectWorkflow(
          {
            context,
      ...
      handleInboundReply: async (
        ...
        const context = deps.getContext();
        const outcome = await deps.processInboundReply(
          {
            context,
            runSandboxTurn: (request) => deps.runSandboxTurn(context, request),
          },
  ...
  const campaignOps = actor({
    ...
    actions: {
      setGlobalKillSwitch: async (c, enabled: boolean) => {
        await deps.getContext().repository.setControlFlag("global_kill_switch", { enabled });
        c.state.lastMutationAt = Date.now();
        return { ok: true, enabled };
      },
      setNoSendsMode: async (c, enabled: boolean) => {
        await deps.getContext().repository.setControlFlag("no_sends_mode", { enabled });
        c.state.lastMutationAt = Date.now();
        return { ok: true, enabled };
      },
      pauseCampaign: async (c, campaignId: string) => {
        const repository = deps.getContext().repository;
      ...
      setLinkedinSourceEnabled: async (
        ...
        await deps.getContext().repository.setCampaignLinkedinSource(input.campaignId, input.enabled);
        c.state.lastMutationAt = Date.now();
        return {
          ok: true,
          ...input,
  ...
  const sandboxBroker = actor({
    ...
    actions: {
      runTurn: async (c, request: DefaultSdrSandboxTurnRequest) => {
        const context = deps.getContext();
        const response = await deps.runSandboxTurn(context, request);
        c.state.runs += 1;
      ...
      enqueueTurn: async (c, request: DefaultSdrSandboxTurnRequest) => {
        const jobId = deps.createId("job");
        const now = Date.now();
      ...
      processQueuedTurn: async (c, input: { jobId: string }) => {
        ...
        const request = parseJsonValue<DefaultSdrSandboxTurnRequest | null>(row.request_json, null);
        if (!request) {
