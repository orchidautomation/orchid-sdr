...
<html lang="en">
  ...
  <body>
    <main class="panel">
      <p class="eyebrow">Trellis / Protected Access</p>
  ...
  <body>
            ...
            <div class="brand">
              <div class="brand-mark">
                ...
                <div class="brand-copy">
                  <p class="brand-kicker">Trellis</p>
    ...
    <script>
      ...
      function readStoredTab() {
        try {
          return localStorage.getItem("trellis-dashboard-tab");
        } catch {
          return null;
        }
      }
      ...
      function writeStoredTab(value) {
        try {
          localStorage.setItem("trellis-dashboard-tab", value);
        } catch {}
      }
      ...
      function renderWorkflowStateChip(row) {
        ...
        return \`<span class="chip \${state.kind}">\${escapeHtml(state.label)}</span>\`;
      }
      ...
      function renderCoreState(state, options) {
        ...
        renderRows("active-threads", visibleActiveThreads, (thread) => \`
          <tr>
            ...
            <td>
              <div class="cell-stack">
                <span>\${escapeHtml(describeWorkflowSummary(thread))}</span>
                <div class="chip-row">\${renderWorkflowStateChip(thread)}\${renderQualificationChips(thread.qualification)}</div>
              </div>
            </td>
            <td>
              <div class="compact-stack">
                <span>\${badge(thread.stage)}</span>
                <span class="feed-subtle">\${escapeHtml(thread.pausedReason || ("next: " + timeLabel(thread.nextFollowUpAt)))}</span>
              </div>
            </td>
        ...
        renderRows("recent-prospects", visibleRecentProspects, (prospect) => \`
          <tr>
            ...
            <td>
              <div class="cell-stack">
                <span>\${escapeHtml(describeWorkflowSummary(prospect))}</span>
                <span class="feed-subtle">\${escapeHtml(prospect.qualification?.engine || prospect.stage)}</span>
                <div class="chip-row">
                  \${renderWorkflowStateChip(prospect)}
